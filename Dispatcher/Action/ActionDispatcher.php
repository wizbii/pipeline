...
class ActionDispatcher implements ActionDispatcherInterface
{
     ...
     */
    protected function runStore($store, $action): void
    {
        $runner = $this->container->get($store->getService(), ContainerInterface::NULL_ON_INVALID_REFERENCE);

        if ($runner === null) {
            throw new StoreNotRunnableException(sprintf("Store '%s' is not runnable: service '%s' does not exist", $store->getName(), $store->getService()));
        }

        if (!$runner instanceof StoreInterface) {
            throw new StoreNotRunnableException(sprintf("Service %s' does not implement '%s'", $store->getService(), StoreInterface::class));
        }
